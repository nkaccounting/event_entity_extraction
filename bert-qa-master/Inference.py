...
def inference_from_half():
    ...
    QAList = [
        QAInput('业绩下滑', '山河智能(002097)遭股东减持700万股湘潭电化(002125)电解锰全线停产每月营收减少1500万'),
        QAInput('实控人股东变更', '佳讯飞鸿:部分董监高拟合计减持逾2%公司股份吉林敖东(000623)实际控制人发生变更'),
        QAInput('不能履职', '成都中禾投资公司老板跑路 专家忧年底或现倒闭潮昆百大A(000560)重要股东累计减持5850万股'),
        QAInput('涉嫌非法集资',
                '新华网北京１０月１８日电（记者李延霞、王文帅）记者１８日从中国保监会获悉，辽宁、浙江保监局近日分别查处了辖区内的琛德保险代理公司负责人何丰以及杭州民丰保险代理有限公司涉嫌非法集资和传销等违法行为'),
        QAInput('实控人股东变更', '三花股份(002050)股东减持772万股 套现8883万元新疆城建(600545)资产重组或涉及控股权变更近5吨进口法瑞康奶粉细菌超标 由法国喜丽雅生产'),
        QAInput('交易违规', '*ST山水(600234)股东黄国忠持股将遭法院拍卖陈发树减持青岛啤酒(600600)被疑内幕交易珠江投资子公司登安全质量违法黑榜 工地存严重隐患'),
    ]
    for qa in QAList:
        data = [
            qa
